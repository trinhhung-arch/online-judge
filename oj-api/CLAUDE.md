...
- **Không nằm trong `/api/v1/`** và **không được lộ ra ngoài tunnel**. Chỉ nghe trên mạng nội bộ.
  Từ 2026-09-23 luật này được ép **trong mã**: `InternalSecretFilter` trả 404 cho mọi request
  tới `/internal` mang header Cloudflare (`CF-Ray`, `CF-Connecting-IP`, `CDN-Loop`), kể cả khi
  đúng secret — vì luật ingress là một regex và nó từng thủng (`//internal/…`). Hệ quả: worker
  **không bao giờ** gọi `/internal` qua Cloudflare; worker ở máy khác phải đi mạng riêng.
